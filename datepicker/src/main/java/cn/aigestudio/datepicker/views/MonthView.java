...
import cn.aigestudio.datepicker.enablejudge.DateLimitJudge;
...
public class MonthView extends View {
    ...
    private boolean isNewEvent,
            ...
            isTodayBgDisplay = true,
            isTodayTextColorDisplay = true,
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isNewEvent) {
                    if (Math.abs(lastPointX - event.getX()) > 100) {
                        ...
                        if (!isAllowVerScroll) {
                            mSlideMode = SlideMode.VER_DISALLOW;
                        } else {
            ...
            case MotionEvent.ACTION_UP:
                ...
                } else if (mSlideMode == SlideMode.VER_DISALLOW) {
    ...
    private boolean isStartDate() {
        return dateLimit != null && centerYear == dateLimit.start.year && centerMonth == dateLimit.start.month;
    }

    private boolean isEndDate() {
        return dateLimit != null && centerYear == dateLimit.end.year && centerMonth == dateLimit.end.month;
    }
...
//        canvas.drawRect(0,0,1080,227,mPaint);
    ...
    private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
        ...
        if (info.isToday && isTodayBgDisplay) {
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        mPaint.setColor(mTManager.colorBgToday());
    ...
    private void drawGregorian(Canvas canvas, Rect rect, DPInfo dpInfo) {
        mPaint.setTextSize(sizeTextGregorian);
        if (dpInfo.isToday && isTodayTextColorDisplay) {
            mPaint.setColor(mTManager.colorTextToday());
        ...
        if (dayEnableJudge != null) {
            if (!TextUtils.isEmpty(dpInfo.strG)) {
                if (!dayEnableJudge.isEnable(new YMDDate(centerYear, centerMonth, Integer.valueOf(dpInfo.strG)))) {
                    mPaint.setColor(mTManager.colorDisEnableDay());
                }
            }
        }
        ...
        if (null != mDPDecor && dpInfo.isReplaceText) {
            String date = centerYear + "-" + centerMonth + "-" + dpInfo.strG;
            mDPDecor.drawReplaceText(canvas, rect.centerX(), y, mPaint, date);
        } else {
    ...
    public void setDayEnableJudge(DayEnableJudge dayEnableJudge) {
        ...
        monthSelect.setDayEnableJudge(dayEnableJudge);
    }
    ...
    public void setTodayBgDisplay(boolean isTodayDisplay) {
        this.isTodayBgDisplay = isTodayDisplay;
    }

    public void setTodayTextColorDisplay(boolean todayTextColorDisplay) {
        isTodayTextColorDisplay = todayTextColorDisplay;
    }
    ...
    public void setDateLimit(DateLimit dateLimit) {
        ...
        if (dayEnableJudge == null) {
            setDayEnableJudge(new DateLimitJudge(dateLimit));
        }
    }
